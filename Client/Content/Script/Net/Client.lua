...
function NetClient:UnpackMsg(data)
    ...
    if self.isReceivedAll == false then
        ...
        if data_size + cache_data_size == self.cacheSize then
            ...
            local bytes = assert(pb.decode("rpc.MsgBase", string.sub(self.cacheData, 7, -1)))
            --print("收到完整,调用: ", id)
        ...
        elseif data_size + cache_data_size > self.cacheSize then
            ...
            local bytes = assert(pb.decode("rpc.MsgBase", body))
            --print("收到多余,调用: ", id)
            self:OnMessageEvent(id, bytes.msg_data)
    ...
    if size > #data then
        ...
        for i=1, 1000 do -- 最多一次拆解1000个包，其余丢弃
            ...
            if #pak == size then
                ...
                local bytes = assert(pb.decode("rpc.MsgBase", string.sub(pak, 7, -1)))
                self:OnMessageEvent(id, bytes.msg_data)
            end
            ...
            if size == #left_data then -- 最后一个包
                ...
                self.isReceivedAll = true
                self.cacheSize = 0
                local bytes = assert(pb.decode("rpc.MsgBase", string.sub(left_data, 7, -1)))
                self:OnMessageEvent(id, bytes.msg_data)
                return 2
